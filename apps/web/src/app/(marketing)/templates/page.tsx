...
export default function MarketingTemplatesPage() {
  return (
    <div className="mx-auto max-w-content px-4 py-16">
      ...
      <p className="mt-3 max-w-2xl text-[color:var(--cv-text-secondary)]">
        Five highly customizable designs — pick one, tune colors and fonts, export ATS-ready PDFs.
      </p>
      <div className="mt-10 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
        {TEMPLATE_CATALOG.map((t) => (
          <div key={t.id} className="rounded-xl border border-border p-4">
            <p className="text-xs uppercase tracking-wide text-[color:var(--cv-text-secondary)]">
              {t.category}
            </p>
            <h2 className="mt-1 text-xl font-semibold">{t.name}</h2>
            <p className="mt-2 text-sm text-[color:var(--cv-text-secondary)]">{t.description}</p>
